...
const AcademicModal: React.FC<TargetModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editUser.id) {
      ...
      console.log(newUser, score, formData.score)
      addUser(newUser)
    }
  ...
  useEffect(() => {
    const defaultQualific = editUser.level || 'BS'
  ...
  useEffect(() => {
    const defaultArea = getAnyOptions('areas')[0] || ''
            ...
            {heading == 'Academic' ? (
              <>
                ...
                {getAnyOptions('scoreType') === 'CGPA' ? (
                  <Form.Group>
                    <RangeSlider name='score' value={score} onChange={handleScore} />
                  </Form.Group>
                ) : (
                  <Form.Group>
                    <Form.Select
